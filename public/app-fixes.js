            ...
            function initializeApp(guestMode, isAdminUser = false, customUserData = null) {
                ...
                if (!guestMode) {
                    ...
                    window._startRealtimeListeners = function() {
                        ...
                        // Global Set to track already-rendered crisis IDs
                        if (!window._renderedCrisisIds) window._renderedCrisisIds = new Set();
                        ...
                        if (!window._crisisListener) {
                            window._crisisListener = db.collection('crisis_reports')
                                ...
                                .limit(20)
                                .onSnapshot(function(snap) {
                                    if (!snap || snap.metadata.fromCache) return;  // Skip cache-only snapshots
                                    snap.docChanges().forEach(function(change) {
                                        ...
                                        if (change.type === 'removed') {
                                            // ── REMOVED: wipe from feed container ───────────────
                                            var _rmId = cr.id;
                                            var fc = document.getElementById('feed-container');
                                            if (fc) {
                                            ...
                                            window._renderedCrisisIds.delete(_rmId);
                                            return;
                                        }
                                        if (change.type === 'added') {
                                            // CRITICAL: Triple-check to prevent duplicates
                                            if (window._renderedCrisisIds.has(cr.id)) {
                                                return;  // Already rendered in memory
                                            }
                                            var fc = document.getElementById('feed-container');
                                            if (!fc) return;
                                            
                                            if (fc.querySelector('[data-post-id="'+cr.id+'"]')) {
                                                window._renderedCrisisIds.add(cr.id);
                                                return;  // Already in DOM
                                            }
                                            
                                            // Mark as rendered BEFORE calling createCrisisPostOnFeed
                                            window._renderedCrisisIds.add(cr.id);
